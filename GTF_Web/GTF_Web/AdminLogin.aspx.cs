...
namespace GTF_Web
{
	public partial class AdminLogin : System.Web.UI.Page
	{
		...
		protected void Button1_Click(object sender, EventArgs e)
		{
			...
			logCon.Open();
			string query = "SELECT COUNT(*) FROM AdminDetails WHERE username='" + TextBoxUser.Text + "'";
			SqlCommand cmd = new SqlCommand(query, logCon);
			int count = Convert.ToInt32(cmd.ExecuteScalar());
			if (count == 1)
			{
				string passQuery = "SELECT password FROM AdminDetails WHERE username='" + TextBoxUser.Text + "'";
